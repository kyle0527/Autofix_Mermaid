...
interface PythonDependencySpec {
  module: string;
  symbols?: string[];
}
...
function inferLanguage(mod: IRModule): 'python' | 'javascript' | 'typescript' | 'unknown' {
...
function resolvePythonModuleName(mod: IRModule, spec: string): string {
...
function extractPythonSymbols(segment: string): string[] {
  return segment
    .replace(/[()]/g, ' ')
    .split(',')
    .map((part) => part.trim())
...
function parsePythonImport(mod: IRModule, raw: string): PythonDependencySpec[] {
  const normalized = raw.replace(/\s+/g, ' ').trim();
  if (!normalized) return [];

  const results: PythonDependencySpec[] = [];
...
function mergeSymbols(existing: string[] | undefined, next: string[] | undefined): string[] | undefined {
...
function extractJSImportSymbols(clause: string | undefined): string[] {
  if (!clause) return [];
...
function parseJSImport(raw: string): JSImportSpec[] {
  const trimmed = raw.trim();
  if (!trimmed) return [];

  const entries = new Map<string, JSImportSpec>();
  const add = (module: string, symbols?: string[]) => {
...
function normalizePythonDependencies(
  mod: IRModule,
  raw: string,
  moduleNames: Set<string>,
...
function normalizeJSImportDependencies(
  mod: IRModule,
  raw: string,
  moduleNames: Set<string>,
  pathToModule: Map<string, string>,
...
function normalizeGenericDependency(
  mod: IRModule,
  raw: string,
  moduleNames: Set<string>,
  pathToModule: Map<string, string>,
...
export function buildDependencyGraph(project: IRProject): DependencyGraph {
