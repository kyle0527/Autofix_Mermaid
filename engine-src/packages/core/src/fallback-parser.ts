...
const FALLBACK_EXTENSION_MAP: Record<string, string[]> = {
  java: ['.java'],
  go: ['.go'],
  csharp: ['.cs'],
  'c#': ['.cs'],
...
function extractBases(header: string): string[] {
...
function isIndexWithinRanges(index: number, ranges?: TextRange[]): boolean {
...
function extractFunctions(
  ...
  const results = new Map<string, IRFunction>();
...
function parseModule(
  ...
  const moduleFunctions = new Map<string, IRFunction>();
  const classRanges: TextRange[] = [];
  const classMap = new Map<string, IRClass>();
  ...
  const moduleContext: FunctionExtractionContext = {
    moduleName,
    filePath,
    parentSource: source,
    scope: 'module',
...
function buildParserMeta(lang: string, runtime?: ParserParseOptions['runtime'], extensions?: string[]): ParserMeta {
  const details: Record<string, unknown> = { lang };
...
export function createFallbackParserPlugin(
  lang: string,
  config: FallbackParserConfig = {},
): ParserPlugin {
  ...
  return {
    ...
    parseProject: async (files: Record<string, string>, options?: ParserParseOptions): Promise<IRProject> => {
