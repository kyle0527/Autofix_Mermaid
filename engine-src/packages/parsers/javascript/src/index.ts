...
function filterJavaScriptEntries(files: Record<string, string>): FileEntry[] {
  return Object.entries(files).filter(([filePath]) =>
    JS_EXTENSIONS.some((ext) => filePath.endsWith(ext))
  );
}
...
async function parseJavaScriptProjectInternal(files: Record<string, string>, options?: ParserParseOptions): Promise<IRProject> {
...
function detectJavaScriptProject(files: Record<string, string>): ParserDetectionResult | null {
...
export async function parseJavaScriptProject(files: Record<string, string>, options?: ParserParseOptions): Promise<IRProject> {
...
function parseWithTreeSitter(
  ...
  for (const [filePath, source] of entries) {
    ...
    function addFunction(fn: IRFunction) {
      if (!functions.some((existing) => existing.id === fn.id)) {
        functions.push(fn);
      }
    }

    function addClass(cls: IRClass) {
      classes.push(cls);
    }

    function addImport(spec: string | undefined) {
      if (!spec) return;
...
async function parseWithWebTreeSitter(entries: FileEntry[], config: WebTreeSitterConfig): Promise<IRProject> {
  ...
  const details: Record<string, unknown> = { grammar: 'tree-sitter-javascript', mode: 'web' };
...
function ensureJavaScriptSyntax(source: string, filePath: string): void {
...
function sanitizeJavaScriptForEval(source: string): string {
  return source
...
export const javascriptParserPlugin: ParserPlugin = {
  lang: 'javascript',
