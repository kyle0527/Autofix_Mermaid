...
const core_1 = require("@diagrammender/core");
...
async function main() {
    ...
    const di = args.indexOf('--diagram');
    const fi = args.indexOf('--format');
    const oi = args.indexOf('--out');
    if (i < 0 || li < 0 || di < 0 || fi < 0 || oi < 0)
        usageAndExit();
    const input = args[i + 1];
    const lang = args[li + 1];
    const diagram = args[di + 1];
    const fmt = (args[fi + 1] || 'mmd');
    const out = args[oi + 1];
    const files = {};
    ...
    if (st.isDirectory())
        ...
        if (!input.endsWith('.py')) {
            console.error('Only .py supported in this MVP');
            process.exit(1);
        }
        files[path.basename(input)] = fs.readFileSync(input, 'utf-8');
    }
    const { code, notes } = await (0, core_1.runPipeline)(files, { lang, diagram, mermaidVersion: 'v11' });
    let outPath = out;
    ...
    if (fmt === 'mmd')
        ...
        fs.writeFileSync(outPath, wrapHTML(code), 'utf-8');
    if (notes.length)
        console.error('Notes:\n' + notes.map(n => ' - ' + n).join('\n'));
    console.error('Saved:', outPath);
}
main().catch(e => { console.error(String((e === null || e === void 0 ? void 0 : e.stack) || e)); process.exit(1); });
