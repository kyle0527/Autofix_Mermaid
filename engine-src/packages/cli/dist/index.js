...
function sanitizeFragmentName(id) {
...
async function main() {
    ...
    const fmt = (fi >= 0 ? args[fi + 1] : undefined);
    const out = args[oi + 1];
    const debug = dbg >= 0;
    const fragmentsDir = fragmentsIdx >= 0 ? args[fragmentsIdx + 1] : undefined;
    if (!input || !diagram || !out)
        usageAndExit();
    if (li >= 0 && !lang)
        usageAndExit();
    if (fi >= 0 && !fmt)
        usageAndExit();
    if (fragmentsIdx >= 0 && !fragmentsDir)
        usageAndExit();
    ...
    if (st.isDirectory())
        ...
        if (!ext) {
            console.error('Input file must have an extension');
            process.exit(1);
        }
    ...
    const pipelineResult = await (0, core_1.runPipeline)(files, {
        lang,
        diagram,
        mermaidVersion: 'v11',
    });
    ...
    if (fragmentsDir) {
        ...
        const manifest = { diagram, fragments: [], links };
