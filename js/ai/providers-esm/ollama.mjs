// ESM variant of Ollama provider
  ...
  const impl = {
    ...
    async analyze(files, options = {}){
      ...
      try {
        ...
        const body = JSON.stringify({ model: options.model || 'qwen2.5:3b', prompt, stream: false, options: { temperature: 0.1 } });
        const resp = await fetch('http://localhost:11434/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
