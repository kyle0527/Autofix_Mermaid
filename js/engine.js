...
// basic analyzers (CFG stub + CallGraph), Mermaid emitters, and fix rules.
// If web-tree-sitter is available and WASM is provided, you can extend `parsePythonProject`
// to use it; this build currently uses a robust regex-based fallback.
...
(function(global){
  ...
  // ---------- Analyzers ----------
  // Simple CFG stub: if we have no body statements, produce start->end only
  let __idc = 0;
  ...
  // Allow passing a pre-parsed IR (e.g., from web-tree-sitter in the worker)
  async function runPipelineIR(ir, opts){
    if (ir && ir.modules){
      for (const mod of Object.values(ir.modules)){
        for (const fn of (mod.functions || [])){
  ...
  global.DiagramMenderCore = { runPipeline, runPipelineIR };
})(self);
