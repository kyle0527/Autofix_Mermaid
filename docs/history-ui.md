...
- **Saved views** – name the current filter/search combination to store it in `localStorage`
  (`autofix.history.views`) and recall it later, even after re-opening the viewer.
- **Pinned entries** – keep important requests always visible at the top of the run card. Pins are
  persisted in `localStorage` (`autofix.history.pins`) and expand automatically when you revisit
  the page.
