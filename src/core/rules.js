...
export const rule_flow_direction = {
    id: 'flow-direction',
    ...
    apply: (ctx) => {
        if (ctx.diag !== 'flowchart')
            return ctx;
        if (/^flowchart\s+(LR|RL|TB|BT|TD)/m.test(ctx.mmd))
            return ctx;
        if (!/^flowchart/m.test(ctx.mmd))
            return ctx; // 不是 flowchart 開頭，略過
        const lines = ctx.mmd.split(/\n/);
        const idx = lines.findIndex(l => /^flowchart\b/.test(l));
        if (idx >= 0)
            lines[idx] = lines[idx] + ' LR';
        return { ...ctx, mmd: lines.join('\n') };
    }
};
export const rule_diagram_kind_infer = {
...
export const rule_dangling_node_prune = {
...
export const rule_label_truncate = {
    id: 'label-truncate',
...
export function applyRules(ctx, rules) {
    let cur = ctx;
    const applied = [];
